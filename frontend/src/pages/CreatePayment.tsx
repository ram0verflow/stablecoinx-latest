...
import { 
  Send, Building2, Globe, Link2, Coins, Target, Zap, 
  ChevronDown, ArrowRight, ShieldCheck, Info, RefreshCw, Wallet, TrendingUp
} from 'lucide-react';
  ...
  const SelectField = ({ label, icon: Icon, field, options, desc }: { label: string; icon: any; field: string; options: string[]; desc?: string }) => (
    <div className="space-y-2">
      <div className="flex items-center justify-between ml-1">
        ...
        {desc && <span className="text-[10px] text-slate-600 font-medium">{desc}</span>}
      </div>
      <div className="relative group">
        <Icon className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-brand-primary transition-colors" />
        <select 
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in pb-20">
      <div className="mb-10 text-center">
        ...
        <h1 className="text-4xl font-extrabold tracking-tight text-white mb-2">Create Settlement</h1>
        <p className="text-slate-500 max-w-md mx-auto">Configure your payment parameters. Every transaction undergoes a mandatory 24-layer compliance scan.</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="glass-card p-8 border-white/5 bg-white/[0.02]">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1">Sender Organization</label>
                    <div className="relative group">
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1">Receiver Organization</label>
                    <div className="relative group">
                        <Building2 className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-brand-primary transition-colors" />
        ...
        <div className="glass-card p-8 border-white/5 bg-white/[0.02]">
            ...
            <div className="grid md:grid-cols-2 gap-6 mt-6">
                <div className="space-y-2">
                    <label className="text-xs font-bold text-slate-500 uppercase tracking-widest ml-1">Settlement Amount</label>
                    <div className="relative group">
                        <Coins className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-brand-primary transition-colors" />
        ...
        <div className="glass-card p-8 border-white/5 bg-white/[0.02]">
            ...
            <div className="grid md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                    <div className="flex items-center justify-between ml-1">
                        <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Urgency Level</label>
                    ...
                    <div className="relative group">
                        ...
                        <select value={form.urgency} onChange={(e) => set('urgency', e.target.value)} className="select-field pl-12 pr-10">
                            {URGENCIES.map((u) => <option key={u} value={u} className="bg-slate-900">{u}</option>)}
                        </select>
                        <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600 pointer-events-none" />
                    </div>
                </div>
                ...
                <div className="grid md:grid-cols-2 gap-6">
                    <div>
                        <div className="flex items-center gap-2 mb-4 ml-1">
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Sender Wallet Address</label>
                        ...
                        <div className="relative group">
                            <Link2 className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-brand-primary transition-colors" />
                            <input 
                    ...
                    <div>
                        <div className="flex items-center gap-2 mb-4 ml-1">
                            <label className="text-xs font-bold text-slate-500 uppercase tracking-widest">Receiver Wallet Address</label>
                            <span className="text-[10px] text-rose-500 font-bold px-1.5 py-0.5 rounded bg-rose-500/10 border border-rose-500/20 uppercase tracking-tighter">Mandatory</span>
                        </div>
                        <div className="relative group">
                            <Link2 className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-500 group-focus-within:text-brand-primary transition-colors" />
